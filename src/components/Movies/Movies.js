        ...
        { Array.isArray(movies) && movies.length!== 0 ?  
                <div>           
                    ...
                    <div className='pagination'>
                        <button disabled={currPage === 1} onClick={()=>setPage(currPage-1)}>Prev</button>
                        <span>- {currPage} -</span>   
                        <button disabled={Math.floor(totalpages/10)===currPage} onClick={()=>setPage(currPage+1)}>Next</button>
                    </div> 
                </div>:
